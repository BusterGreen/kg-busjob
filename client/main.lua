...
local function GetDeliveryLocation()
    ...
    PolyZone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inRange = true
            exports["kg-core"]:DrawText(Lang:t('info.busstop_text'), 'rgb(220, 20, 60)')
            CreateThread(function()
                repeat
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        KGCore.Functions.Notify(Lang:t('success.dropped_off'), 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
                        ...
                        resetNpcTask()
                        nextStop()
                        TriggerEvent('kg-busjob:client:DoBusNpc')
...
local function busGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            header = v.label,
            params = {
                event = "kg-busjob:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("kg-busjob:client:TakeVehicle", function(data)
    local coords = Config.Location
    if (BusData.Active) then
        KGCore.Functions.Notify(Lang:t('error.one_bus_active'), 'error')
        return
    else
...
RegisterNetEvent('kg-busjob:client:DoBusNpc', function()
    if whitelistedVehicle() then
        if not NpcData.Active then
            ...
            KGCore.Functions.Notify(Lang:t('info.goto_busstop'), 'primary')
                    ...
                    CreateThread(function()
                        repeat
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                TriggerServerEvent('kg-busjob:server:NpcPay')
                                exports["kg-core"]:HideText()
                                PolyZone:destroy()
                                break
                            end
            ...
            KGCore.Functions.Notify(Lang:t('error.already_driving_bus'), 'error')
        end
    else
        KGCore.Functions.Notify(Lang:t('error.not_in_bus'), 'error')
    end
end)
            ...
            if isPointInside then
                ...
                CreateThread(function()
                    repeat
                        Wait(5)
                        if not inVeh then
                            exports["kg-core"]:DrawText(Lang:t('info.busstop_text'), 'left')
                            ...
                            exports["kg-core"]:DrawText(Lang:t('info.bus_stop_work'), 'left')
                            if IsControlJustReleased(0, 38) then
                                if (not NpcData.Active or NpcData.Active and NpcData.NpcTaken == false) then
                                    ...
                                    KGCore.Functions.Notify(Lang:t('error.drop_off_passengers'), 'error')
                                end
                            end
                        end
                    until not inRange
